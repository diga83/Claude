...
const STORAGE_KEY = 'gcse_cs_progress_v2';
const THEME_KEY = 'gcse_cs_theme';
const BOOKMARKS_KEY = 'gcse_cs_bookmarks_v1';
const TIMER_KEY = 'gcse_cs_timer_v1';
...
function renderSidebar() {
  ...
  let html = `
    ...
    <h3>Tools</h3>
    ${navItem('tools.html', 'CS calculators', '🧮', '#0F766E', 'tools')}
...
const NOTES_KEY = 'gcse_cs_notes_v1';
...
const PROFILE_KEY = 'gcse_cs_profile_v1';
...
function showWelcomeScreen() {
  ...
  style.textContent = `
    #welcomeOverlay {
      ...
      inset: 0;
      z-index: 9999;
      background: linear-gradient(135deg, #064E3B 0%, #6D28D9 60%, #DC2626 100%);
      display: grid;
      place-items: center;
      padding: 1rem;
    ...
    .welcome-mark {
      ...
      width: 64px;
      height: 64px;
      background: linear-gradient(135deg, #064E3B, #6D28D9);
      color: white;
      border-radius: 18px;
      font-family: var(--font-display);
      font-weight: 800;
...
const TOUR_KEY = 'gcse_cs_tour_seen_v1';
...
const TOUR_STEPS = [
  ...
  },
  {
    ...
    body: `That's the basics! Anything else you discover (the floating Pomodoro timer, mind maps, glossary self-test...) is icing on the cake. Good luck with your GCSE preparation!`,
    target: null
  }
];
...
function showChapterFeaturesHint() {
  const chId = getCurrentChapterId();
  if (!chId) return;
  const HINT_KEY = 'gcse_cs_features_hint_seen_v2';
...
function dismissChapterHint(btn) {
  try { localStorage.setItem('gcse_cs_features_hint_seen_v2', 'yes'); } catch(e) {}
